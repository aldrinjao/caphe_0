...
import 'package:caphe_0/widgets/item_card.dart';
import 'package:caphe_0/utils/questionaire_utils.dart';
import 'package:caphe_0/utils/item.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';

class QuestionairePage extends StatefulWidget {
  const QuestionairePage({this.name, this.user, this.location});
...
class QuestionairePageState extends State<QuestionairePage> {
  ...
  Widget build(BuildContext context) {
    //fuction to check if it's a number
              ...
              child: Padding(
                  ...
                  child: GridView.count(
                    ...
                    children: List.generate(currentList.length, (index) {
                      return ItemCard(
                          'assets/'+kind+"/"+currentList[index].location,
                          currentList[index].name,
                          () => this.setState(() { //this is a callback function when the ItemCard is pressed
                                i++;
                                if(i == 1) kind = currentList[index].name;
                                if (isNumeric(currentList[index].value)) {
                                  Navigator.popUntil(context, (Route<dynamic> route) => route.isFirst);
                                  addValue(kind, widget.name, int.parse(currentList[index].value), widget.location);
                                }else {
                                  state = currentList[index].value;
                                  currentList = questionaire.nextList(state,kind);
                                }
                              }));
                    }),
                  )),
  ...
  void addValue(String species, String name, int bbch, String location) async{
    try {
      await db
          ...
          .updateData({"records": FieldValue.arrayUnion([{'date': DateTime.now(), 'species': species, 'location': location,'name': name, 'bbch': bbch}])});
    } catch (e) {
      print(e.message);
    }
